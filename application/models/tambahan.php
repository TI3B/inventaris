...
class tambahan extends CI_Model{
    ...
    function getKodeBarang(){
        $q = $this->db->query("select MAX(RIGHT(kd_baranggudang,3)) as kd_max from tbl_baranggudang");
        $kd = "";
        if($q->num_rows()>0){
            foreach($q->result() as $k){
                $tmp = ((int)$k->kd_max)+1;
    ...
    public function getTambahStok($kd_baranggudang,$tambah)
    {
        $q = $this->db->query("select stok from tbl_baranggudang where kd_baranggudang='".$kd_baranggudang."'");
        $stok = "";
    ...
    public function getKurangStok($kd_baranggudang,$kurangi)
    {
        $q = $this->db->query("select stok from tbl_baranggudang where kd_baranggudang='".$kd_baranggudang."'");
        $stok = "";
        foreach($q->result() as $d)
    ...
    public function getKembalikanStok($kd_baranggudang)
    {
        $q = $this->db->query("select stok from tbl_baranggudang where kd_baranggudang='".$kd_baranggudang."'");
        $stok = "";
        foreach($q->result() as $d)
    ...
    function getBarangJual(){
        return $this->db->query ("SELECT * from tbl_baranggudang where stok > 0")->result();
    }
    ...
    function getBarangPenjualan($id){
        return $this->db->query("
                select a.kd_baranggudang,a.qty,b.nm_baranggudang,b.harga
                from tbl_penjualan_detail a
                left join tbl_baranggudang b on a.kd_baranggudang=b.kd_baranggudang
                where a.kd_penjualan = '$id'")->result();
    }
